...
const tabWithCounter = document.querySelector('button[data-goods-counts]');

const tabs = document.querySelectorAll('button.tab');
addClickListeners(tabs, clickHandler);

function clickHandler(event) {
	const activeTab = document.querySelector(`button[data-id=${activeTabId}]`);

	activeTab.classList.toggle('active');
	event.target.classList.toggle('active');

	activeTabId = event.target.dataset.id;
...
function addInCartHandler(product) {
	return () => {
		...
		tabWithCounter.dataset.goodsCounts = fullSize;
	}
}
...
function getActiveTab() {
	return document.querySelector(`button[data-id=${activeTabId}]`)
}
...
function renderCart() {
	...
	`;
		...
		const button = document.createElement('button');
		button.classList.add('cart-item-delete');
